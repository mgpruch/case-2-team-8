...
author: "Allison Li, Ben Rubin, Ishaan Maitra, Morgan Pruchniewski"
...
# select up to 7 gymnasts with highest mean scores for each country
w_avs_all <- women_gym_data %>%
  group_by(Country, name) %>%
  summarise(mean = mean(Score)) %>% 
  top_n(7, mean)
m_avs_all <- men_gym_data %>%
  group_by(Country, name) %>%
  summarise(mean = mean(Score)) %>% 
  top_n(7, mean)
```
...
us_team_men <- men_name_list
us_team_women <- women_name_list
```

```{r total medal score, warning=FALSE, message=FALSE, include=FALSE, eval=FALSE}
...
for (iter in 1:num_iterations) {
  ...
  men_total_medal_score = append(men_total_medal_score, total_medal)
}
```

```{r total medal score2, warning=FALSE, message=FALSE, include=FALSE, eval=FALSE}
# score 3 for gold, 2 for silver, 1 for bronze
women_total_medal_score = list() # store possible total medal scores for 1000 iterations
for (iter in 1:num_iterations) {
  total_medal = 0
  ...
  women_total_medal_score = append(women_total_medal_score, total_medal)
}
```
...
In Table 3 below, we show how each of the members on the men's team selected to be included in the simulation perform across all four metrics.
...
In Table 4 below, we provide a table showing how each of the members on the women's team selected to be included in the simulation perform across all four metrics.
...
From the table, we were able to pinpoint Simone Biles as the clear top performer as an all-around athlete, but following was Jordan Chiles with a high composite score that also warrants a spot as an all-around athlete. Zoe Miller and Kaliya Lincoln present more interesting scenarios where they performs highly in few events, which could be valuable if they are projected to medal gold or silver in that single event. On the other hand, outside of Biles and Chiles, Jade Carey, Joscelyn Roberson, and Kayla Dicello perform consistently high - but not highest - on a larger range of events.
...
Our preliminary simulation for medals provides us a way to simulate how our team would perform against other countries. Using the methodology proposed to create an international team to test the US team against, we are able to see how our 2 lineups would score against other teams.

```{r median medal score, include=FALSE, eval=FALSE}
median(unlist(men_total_medal_score))
sd(unlist(men_total_medal_score))
median(unlist(women_total_medal_score))
sd(unlist(women_total_medal_score))
```

Using a scoring system of 3 points for Gold, 2 points for Silver, and 1 point for Bronze, our men's team gets a median medal score of 8 points, a minimum of 6 points, and a maximum of 9 points. It has a standard deviation of 1. Our women's team gets a median medal score of 9 points in the simulation with lower standard deviation of 0.72, a minimum of 8 points, and a maximum of 12 points.
